...
async function deleteScreenshots(components?: string[]) {
  ...
  if (components?.length) {
    const updatedScreenshots = master.screenshots.filter(screenshot => !new RegExp(`.*(${components.join('|')})\\.e2e\\.ts$`, 'g').test(screenshot.testPath));
    const masterNeedsUpdate = updatedScreenshots.length !== master.screenshots.length;
    if (masterNeedsUpdate) {
      master.screenshots = updatedScreenshots;
      await fs.writeFile('screenshot/builds/master.json', JSON.stringify(master, undefined, 2) + '\n', { encoding: 'utf8', flag: 'w' });
  ...
  for (const filePath of filePaths) {
    ...
    if (!referencedFileNames.find(referencedFileName => referencedFileName === fileName)) {
...
yargs(process.argv.slice(2))
  ...
  .command(
    ...
    yargs => {
      yargs
        .example('$0 rm --components ld-button ld-select', 'Deletes all obsolete screenshots as well as all screenshots created with e2e tests for ld-button and ld-select.')
        .alias('c', 'components')
        .array(['components'])
        .help('h')
        .alias('h', 'help');
